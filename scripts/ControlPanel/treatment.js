...
function ordersListMore() {
    ...
    const inReceptionButton = document.querySelector('#inReception')
    const forWashingButton = document.querySelector('#forWashing')
    const toDryButton = document.querySelector('#toDry')
    inReceptionButton.classList.remove('active')
    forWashingButton.classList.remove('active')
    toDryButton.classList.remove('active')
    inReceptionButton.addEventListener('click', () => {
        state = []
        ordersPageQuery.forEach(list => {
            list.orders.forEach(order => {
                state.unshift(order)
        ...
        state = state.filter(order => order.status === "In reception")
        inReceptionButton.classList.add('active')
        forWashingButton.classList.remove('active')
        toDryButton.classList.remove('active')
        updateSection()
    })
    forWashingButton.addEventListener('click', () => {
        state = []
        ordersPageQuery.forEach(list => {
            list.orders.forEach(order => {
                state.unshift(order)
        ...
        state = state.filter(order => order.status === "To wash")
        inReceptionButton.classList.remove('active')
        forWashingButton.classList.add('active')
        toDryButton.classList.remove('active')
        updateSection()
    })
    toDryButton.addEventListener('click', () => {
        state = []
        ordersPageQuery.forEach(list => {
            list.orders.forEach(order => {
                state.unshift(order)
        ...
        state = state.filter(order => order.status === "To dry")
        inReceptionButton.classList.remove('active')
        forWashingButton.classList.remove('active')
        toDryButton.classList.add('active')
        updateSection()
    })
